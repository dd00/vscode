...
import { ExposedPortInfo, OnPortExposedAction, PortsStatus, PortsStatusRequest, PortsStatusResponse, PortVisibility, TunneledPortInfo } from '@gitpod/supervisor-api-grpc/lib/status_pb';
...
export async function activate(context: vscode.ExtensionContext) {
	...
	interface TunneledPort extends PortsStatus.AsObject {
		tunneled: TunneledPortInfo.AsObject
	}
	function isTunneledPort(port: PortsStatus.AsObject | undefined): port is TunneledPort {
		return !!port?.tunneled;
	}
	interface TunneledGitpodWorkspacePort extends GitpodWorkspacePort {
		status: TunneledPort
	}
	function isTunneledGitpodWorkspacePort(port: GitpodWorkspacePort | undefined): port is TunneledGitpodWorkspacePort {
		return port instanceof GitpodWorkspacePort && isTunneledPort(port.status);
	}
	interface ExposedPort extends PortsStatus.AsObject {
		exposed: ExposedPortInfo.AsObject
	}
	function isExposedPort(port: PortsStatus.AsObject | undefined): port is TunneledPort {
		return !!port?.exposed;
	}
	...
	if (vscode.env.uiKind === vscode.UIKind.Web) {
		...
		context.subscriptions.push(gitpodWorkspaceTreeDataProvider.onDidChangeTreeData(() => {
			const toDispose = new Set<number>(gitpodWorkspaceTreeDataProvider.tunnels.keys());
			for (const port of gitpodWorkspaceTreeDataProvider.ports.ports.values()) {
				if (isTunneledGitpodWorkspacePort(port) && port.status.tunneled.visibility !== TunnelVisiblity.NONE) {
					toDispose.delete(port.portNumber);
					const options: vscode.TunnelOptions = {
						remoteAddress: {
							host: 'localhost',
							port: port.portNumber
						},
						localAddressPort: port.status.tunneled.targetPort,
						public: port.status.tunneled.visibility === TunnelVisiblity.NETWORK
					};
					if (!port.tunnel) {
						Promise.resolve(vscode.workspace.openTunnel(options)).catch(() => { });
					} else if (!!port.tunnel.public !== options.public) {
						toDispose.add(port.portNumber);
					}
				}
			}
			vscode.commands.executeCommand('gitpod.disposeClosedTunnels', [...toDispose]);
		}));
		// TODO(ak) hide behind a command
		vscode.commands.executeCommand('setContext', 'forwardedPortsViewEnabled', true);
		context.subscriptions.push(vscode.commands.registerCommand('gitpod.openTunnel', async (tunnelOptions: vscode.TunnelOptions, _tunnelCreationOptions: vscode.TunnelCreationOptions) => {
			const request = new TunnelPortRequest();
		...
		context.subscriptions.push(vscode.commands.registerCommand('gitpod.closeTunnel', async (port: number) => {
